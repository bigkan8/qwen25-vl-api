...
This server loads the Qwen2.5-VL-3B-Instruct model and exposes endpoints to analyze images.
"""
...
app = FastAPI(
    title="Qwen VL API",
    description="API for the Qwen2.5-VL-3B-Instruct model to analyze images",
    version="1.0.0"
)
...
async def root():
    """Root endpoint"""
    return {"message": "Qwen2.5-VL-3B-Instruct API is running"}
